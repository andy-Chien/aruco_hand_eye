...
class MarkerPosture():
    def __init__(self, name):
        ...
        self.rvecs = None 
        self.tvecs = None
        self.rvecs_arr = np.zeros((100, 3))
        self.tvecs_arr = np.zeros((100, 3))
        ...
        #aruco topic publisher
        # self.aruco_pub = rospy.Publisher('aruco_tracker/transform', TransformStamped, queue_size=1)
        ...
        if ret:
            ...
            if ids is not None and len(ids) > 0:
                ...
                for _id, rvec, tvec in zip(ids, self.rvecs, self.tvecs):
                    ...
                    # res.transform.translation.x = tvec[0][0]
                    # res.transform.translation.y = tvec[0][1]
                    # res.transform.translation.z = tvec[0][2]
                    # res.transform.rotation.x = quaternion[0]
                    # res.transform.rotation.y = quaternion[1]
                    # res.transform.rotation.z = quaternion[2]
                    # res.transform.rotation.w = quaternion[3]
                    # res.header.frame_id = 'ar_marker'
                    # self.aruco_pub.publish(res)
